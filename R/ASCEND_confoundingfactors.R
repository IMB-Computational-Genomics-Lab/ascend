...
#' @export
RegressGene <- function(gene = NULL, covariate.matrix = NULL, expression.matrix = NULL) {
...
#' it from a AEMSet object with the GetExpressionMatrix function.
...
RemoveConfoundingFactors <- function(expression.matrix, candidate.genes = list()) {
    ...
    # Work with the matrix as a data frame Insert checks for matrix to dataframe
    ...
    # Parallel Process - Looping the regression through every gene Regression can be done for all genes present in the matrix or a subset of genes Can use all
    # of the matrix or part of the matrix
    print("Regressing gene...")
    data.resid <- BiocParallel::bplapply(colnames(expression.matrix), RegressGene, gene = x, covariate.matrix = covariate.mtx, expression.matrix = expression.matrix)
