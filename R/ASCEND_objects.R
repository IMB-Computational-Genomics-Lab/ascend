  ...
  if (length(object@Conditions) > 0) {
    ...
    for (condition in object@Conditions) {
      if (names(condition) != 2) {
        msg <- "Please ensure you have grouped the cells into positive and negative condition groups."
        errors <- c(errors, msg)
      }
    }
  }
...
#' @slot PCA Objects related to dimension reduction, such as a PCA matrix, a reduced PCA matrix and a list of percentage variance values per principle component.
#' @slot TSNE TSNE-reduced matrices used for plotting.
#' @slot Clusters Objects related to clustering, including a distance matrix, a hclust object, cell identifiers and their associated cluster.
#' @slot DifferentialExpression Differential expression results from DESeq.
...
#'
setClass("AEMSet", representation(ExpressionMatrix = "Matrix", GeneAnnotation = "data.frame",
                                  BatchInformation = "list", Controls = "list", PCA = "list", TSNE = "list", Conditions = "list",
                                  Clusters = "list", DifferentialExpression = "list", Metrics = "list", Log = "list"), 
                                  prototype(ExpressionMatrix = Matrix::Matrix(0, nrow = 0, ncol = 0, sparse = TRUE),
                                                           GeneAnnotation = data.frame(matrix(nr = 0, nc = 0)),
                                                           BatchInformation = list(), Controls = list(),
                                                           Conditions = list(), PCA = list(), TSNE = list(),
                                                           Clusters = list(), DifferentialExpression = list(),
                                                           Metrics = list(), Log = list()), validity = CheckAEMSet)
...
setMethod("show", signature("AEMSet"), function(object) {
  ...
  expression.matrix.str <- sprintf("Expression Matrix: %i cells and %i genes",
                                   n.cells, n.genes)
  ...
  for (control.name in names(object@Controls)) {
    ...
    control.str <- sprintf("%s: %i genes", control.name, n.control.genes)
    print(control.str)
  }
...
#'
NewAEMSet <- function(ExpressionMatrix = NULL, GeneAnnotation = NULL, BatchInformation = NULL,
                      Controls = list(Mt = list(), Rb = list())) {
  # Check that we have the essential arguments
  arg.check <- list(ExpressionMatrix = missing(ExpressionMatrix), Controls = missing(Controls))
  ...
  if (is.null(BatchInformation)) {
    BatchInformation <- setNames(rep(1, ncol(ExpressionMatrix)), as.list(colnames(ExpressionMatrix)))
  ...
  sea.set <- new("AEMSet", ExpressionMatrix = sparse.matrix, GeneAnnotation = GeneAnnotation,
                 BatchInformation = BatchInformation, Controls = Controls)
...
setMethod("GenerateMetrics", signature("AEMSet"), function(object) {
  ...
  control.counts <- BiocParallel::bplapply(names(control.list), GenerateControlMetrics, expression.matrix = expression.matrix, control.list = control.list, total.counts = total.counts)

  # Unpackage stats
  unpacked.control.counts <- unlist(control.counts, recursive = FALSE)
  percentage.lists.counts.list <- lapply(names(unpacked.control.counts),
                                         function(x) unpacked.control.counts[[x]][["PercentageTotalCounts"]])
  names(percentage.lists.counts.list) <- names(unpacked.control.counts)
  control.transcript.counts.list <- lapply(names(unpacked.control.counts),
                                           function(x) unpacked.control.counts[[x]][["ControlTranscriptCounts"]])
  names(control.transcript.counts.list) <- names(unpacked.control.counts)

  # Calculate feature counts (Exclude controls)
  control.bool <- rownames(expression.matrix) %in% (unlist(object@Controls,
                                                           use.names = FALSE))
  ...
  if (any(control.bool)) {
    ...
    percentage.lists.counts.list <- lapply(percentage.lists.counts.list,
                                           function(x) ifelse(is.na(x), 0, x))
    control.transcript.counts.list <- lapply(control.transcript.counts.list,
                                             function(x) ifelse(is.na(x), 0, x))
  }

  total.features.counts.per.cell <- Matrix::colSums(endogenous.exprs.mtx !=
                                                      0)
  ...
  metrics.list <- list(TotalCounts = total.counts, TotalFeatureCountsPerCell = total.features.counts.per.cell,
                       TotalExpression = total.expression, TopGeneList = top.gene.list,
                       TopGenesPercentage = top.genes.percentage, ControlTranscriptCounts = control.transcript.counts.list,
                       PercentageTotalCounts = percentage.lists.counts.list, AverageCounts = average.counts,
                       GenesPerCell = genes.per.cell, CellsPerGene = cells.per.gene, CountsPerGene = counts.per.gene,
...
#'
setGeneric(name = "UpdateControls", def = function(object, gene.list) {
...
setMethod("UpdateControls", signature("AEMSet"), function(object, gene.list) {
  ...
  if (length(gene.list) > 0) {
    check.controls <- unlist(gene.list) %in% rownames(object@ExpressionMatrix)
...
#'
setGeneric(name = "GetBatchInfo", def = function(object){
...
#'
setGeneric(name = "UpdateBatchInfo", def = function(object) {
...
setMethod("UpdateBatchInfo", signature("AEMSet"), function(object) {
  ...
  updated.barcode.list <- current.barcode.list[names(current.barcode.list) %in%
                                                 present.barcodes]
...
#'
setGeneric(name = "ConvertGeneAnnotation", def = function(object, old.annotation,
                                                          new.annotation) {
...
setMethod("ConvertGeneAnnotation", signature("AEMSet"), function(object) {
  ...
  updated.control.list <- gene.annotation[, new.annotation][which(gene.annotation[, old.annotation] %in% control.list$Mt)]
...
#'
setGeneric(name = "ReplaceExpressionMatrix", def = function(x, object) {
...
#'
setGeneric(name = "SubsetBatch", def = function(object, x) {
...
setMethod("SubsetBatch", signature("AEMSet", "numeric"), function(object, x) {
  ...
  if (!is.null(object@Clusters$DistanceMatrix)) {
    distance.matrix <- as.matrix(object@Clusters$DistanceMatrix)[barcode.list,
                                                                 barcode.list]
...
#'
setGeneric(name = "SubsetCluster", def = function(object, x) {
...
setMethod("SubsetCluster", signature("AEMSet", "numeric"), function(object, x) {
  ...
  # Retrieve cells that belong to this batch from the AEMSet obejct
  cluster.list <- object@Clusters$Clusters[which(object@Clusters$Clusters ==
                                                   as.character(x))]
