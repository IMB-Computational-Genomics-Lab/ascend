...
```{r BiocParallelSetupUnix, eval=TRUE}
...
As these files are tab-separated variables, we can read them into R as CSV files.
...
```{r}
mito.genes <- genes$gene_name[grep("^mt-", ignore.case=TRUE, genes$gene_name)]
ribo.genes <- genes$gene_name[grep("^rps|^rpl", ignore.case=TRUE, genes$gene_name)]
spike.ins <- genes$gene_name[grep("^ercc", ignore.case=TRUE, genes$gene_name)]
...
```{r}
ascend.obj <- NewAEMSet(ExpressionMatrix = expression.matrix, GeneAnnotation = genes, BatchInformation = batch.information, Controls = control.list)
ascend.obj
```
...
```
This plot represents the distribution of library sizes across all of the cells in the dataset. 

#### Number of expressed genes per cell
```{r fig2, fig.height = 4, fig.width= 6, echo = FALSE}
print(raw.qc.plots$FeatureCountsPerCell)
```
This plot represents the number of genes a cell expresses, and excludes controls.

#### Average Gene Expression
```{r fig3, fig.height = 4, fig.width = 10, echo = FALSE}
grid.arrange(raw.qc.plots$AverageGeneCount, raw.qc.plots$Log2AverageGeneCount, raw.qc.plots$Log10AverageGeneCount, ncol = 3)
```
These plots represent the average count of a gene. The first plot indicates that this dataset is comprised mostly of zero-expression values that is consistent with single-cell RNASeq. Log-transforming the values gives us a better idea of the distribution of the values.

#### Proportion of control gene expression to total gene expression



#### Total mapped reads and genes per sample
```{r fig6, fig.height=4, fig.width=5}
print(raw.qc.plots$LibSizePerSample)
```

This plot represents the number of mapped reads and genes per sample.

#### Top genes per sample
```{r fig7, fig.height = 4, fig.width = 6}
print(raw.qc.plots$TopGenesPerSample)
```
...
```{r fig9, fig.width = 10, fig.height = 4}
...
```{r fig8, fig.height = 10, fig.width=6}
print(raw.qc.plots$TopGenes)
```

In some cases, the control genes dominate the expression of a cell, as shown in these plots.
```{r fig5, fig.height = 8, fig.width = 8, echo = FALSE}
grid.arrange(raw.qc.plots$ControlPercentageSampleCounts$Mt, raw.qc.plots$ControlPercentageSampleCounts$Rb, raw.qc.plots$ControlPercentageSampleCounts$SpikeIns, ncol = 2)
```
```{r fig4, fig.height = 4, fig.width = 10, echo = FALSE}
grid.arrange(raw.qc.plots$ControlPercentageTotalCounts$Mt, raw.qc.plots$ControlPercentageTotalCounts$Rb, raw.qc.plots$ControlPercentageTotalCounts$SpikeIns, ncol = 3)
```
...
You may also want to remove these controls completely for further analysis. This is best done after normalisation.

#### Filtering by expression
We also need to remove low-abundance genes, which can be seen on the AverageGeneCount plots.
...
```{r}
# Print log output - make a nice function for this
filtered.object
```
...
```{r}
# Print removed barcodes
```

We can also use the PlotGeneralQC function to generate a new set of plots, so we can see how our filtering has affected the dataset. (Rework this bit - could be better.)
```{r}
...
We can see how this has affected our dataset by using a series of plots generated by PlotNormalisationQC. This function will plot the expression levels of all genes, in addition to a list of genes provided by the user. If the user hasn't provided a list, the function will plot a gene chosen randomly. In this case, we will plot the expression counts for GAPDH, ERH and BAT1.

```{r}
rle.qc <- PlotNormalisationQC(original = filtered.object, normalised = rle.obj, gene.list = list("GAPDH", "ERH", "BAT1"))
```
```{r fig10, fig.width=10, fig.height = 4}
grid.arrange(rle.qc$LibsizeHistograms$Original, rle.qc$LibsizeHistograms$Normalised, ncol = 2)
```

```{r fig11, fig.width=10, fig.height = 4}
print(rle.qc$GeneScatterPlots$ERH$Original) 
print(rle.qc$GeneScatterPlots$ERH$Normalised)
```
#### scranNormalise
...
print(scran.qc$LibSizeHistograms$Original)
print(scran.qc$LibSizeHistograms$Normalised)
```
...
We can also use these results to generate a PCA plot. Here we are colouring by batch.
```{r}
condition.list <- GetBatchInfo(reduced.pca)
pca.plot <- PlotPCA(reduced.pca, dim1 = 1, dim2 = 2, condition.list = condition.list)
print(pca.plot)
```
...
```{r}
tsne.plot <- PlotTSNE(reduced.pca, PCA = TRUE, condition.list = condition.list, seed = 1, perplexity = 30, theta = 0.25)
print(tsne.plot$Plot)
```

This function also returns a table containing the data used in the plot.
```{r}
print(tsne.plot$TSNE)
```

You can also call the TSNE function manually. For example, we want to reduce the data to three dimensions so we can generate a 3D plot.
```{r, eval = FALSE}
tsne.df <- RunTSNE(reduced.pca, PCA = TRUE, dimensions = 3)
```

#### Multi-dimensional Scaling(MDS) Plot
Another way to represent this data is with a MDS plot. This method is time-consuming as it runs cmdscale to further condense the data.
```{r}
mds.plot <- PlotMDS(reduced.pca, PCA = TRUE, dim1 = 1, dim2 = 2, condition.list = condition.list)
print(mds.plot)
```
...
We can track the stability of clusters across each height with PlotStability and PlotStabilityDendro.
```{r}
PlotStability(clustered.obj)
PlotStabilityDendro(clustered.obj)
```
...
You can also compare each cluster to other clusters using RunClusterDiffExpression.
