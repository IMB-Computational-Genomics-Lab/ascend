...
This vignette uses the iPSC Retinal Ganglion Cells dataset, that can be loaded into R with the following function:
...
We can also add other information to this data frame. The cells in this dataset were sorted for expression of the THY1 protein. This corresponds to the batch identifiers that we have just pulled out from the barcodes.

```{r LabelTHY1Cells}
thy1.expression <- barcodes$batch
thy1.expression <- thy1.expression == 1
barcodes$THY1 <- thy1.expression
barcodes[1:5,]
```
...
`PlotNormQC` will generate a series of plots for the review of the normalisation process. This function can only be used if you have retained the un-normalised `AEMSet`. You can also review the expression of genes you are interested in; in this case, we will look at the expression of GAPDH and MALAT1 as they are considered 'housekeeping' genes.
...
```{r ControlRemovalPlot1}
print(filtered.qc.plots$TopGenes)
```

As you can see, ribosomal genes dominate gene expression, even after filtering. What does the dataset look like without these control genes? 
...
Please note that this has already been done as a part of the `scranNormalisation` process.

### Dimension Reduction
We have filtered our dataset down to 1235 cells and 32904 genes. We can reduce this dataset further by using *Principal Component Analysis (PCA)* to identify genes that are major contributors to variation.
...
pca.variance <- PlotPCAVariance(pca.set, 50)
print(pca.variance)
```
...
```{r ReduceDimensions}
pca.set <- ReduceDimensions(pca.set, 20)
```
...
The rand index is stable in more than 50% of tree-cut heights that correspond to the lowest number of clusters. Therefore, the optimal number of clusters is 2.
...
As we had just clustered the data, let's examine what genes are differentially expressed between clusters 1 and 2.
...
```{r RemoveCluster}
clean.set <- SubsetCluster(clustered.set, 1)
