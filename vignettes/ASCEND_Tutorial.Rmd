...
# For those running the script elsewhere
# data("RGC_scRNASeq")
```
...
```{r ControlRemovalPlot1, fig.width = 5, fig.height = 4, fig.align="center", warning = FALSE}
...
We can view these results as a volcano plot with the `RunDEVolcano` function. Usually, we run this function with labels set to FALSE as there are too many points close together, resulting in overlapping labels. We have set labels to TRUE so we can see which genes are of interest.
